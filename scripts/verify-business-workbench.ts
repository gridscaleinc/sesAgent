...
try {
  ...
  const followSource = createRedactedManualJobCaseSource({ subject: 'Followup Java project', body: '必須スキル：Java / SQL' }, 'fa892f81-f9d4-42cf-8050-e68ad880ccad', [])
  const followDraft = extractJobCaseDraft(followSource.source, 'fb892f81-f9d4-42cf-8050-e68ad880ccad')
  repository.saveRedactedJobCaseSourceAndDraft(followSource.redaction.session, followSource.redaction.mappings, followSource.source, followDraft)
  const followReview = repository.getJobCaseReview(followDraft.reviewId)!
  const confirmedCase = repository.confirmJobCaseReview({ reviewId: followDraft.reviewId, reviewRevision: followReview.reviewRevision, privacyReviewed: true, fields: followReview.fields.map((field) => ({ key: field.key, value: field.value, confirmed: true })) }, 'test-hr', 'Test HR')
  repository.setCandidateBusinessState({ ...state, profileVersion: review.profile!.version, status: 'available' }, 'test-hr')
  const currentMessage = { ...updatedMessage, profileVersion: review.profile!.version, caseContext: { reviewId: followDraft.reviewId, version: confirmedCase.jobCase!.version } }
  assert.equal(repository.validatePersonnelMessage(currentMessage).caseContext?.reviewId, followDraft.reviewId)
  assert.throws(() => repository.validatePersonnelMessage({ ...currentMessage, caseContext: { ...currentMessage.caseContext, version: 99 } }), /更新/)
  ...
  const follow = repository.saveBusinessFollowUp({ documentId, reviewId: followDraft.reviewId, expectedRevision: 0, status: 'contacted', note: 'Synthetic contact note', nextStep: 'Await reply' }, 'test-hr')
  ...
  assert.throws(() => repository.saveBusinessFollowUp({ documentId, reviewId: followDraft.reviewId, expectedRevision: 0, status: 'replied', note: 'stale', nextStep: '' }, 'test-hr'), /更新/)
  ...
  const reply = repository.saveBusinessFollowUp({ documentId, reviewId: followDraft.reviewId, expectedRevision: 1, status: 'replied', note: 'Synthetic reply', nextStep: 'Schedule interview' }, 'test-hr')
  assert.equal(reply.events.length, 2)
  assert.notEqual(repository.previewCandidateDeletion(documentId).confirmationHash, beforeFollowEditDeletion.confirmationHash, 'editing followups invalidates an old deletion preview')
  repository.close()
  repository = new EncryptedApplicationRepository({ path: databasePath, databaseKey, mappingKey })
  ...
  db.pragma('legacy=4')
  db.key(databaseKey)
  for (const table of ['business_progress_mail', 'gmail_business_intake', 'business_followups', 'business_feed_marks', 'candidate_business_states', 'personnel_templates', 'personnel_copies']) db.exec(`DROP TABLE ${table}`)
