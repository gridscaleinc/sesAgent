...
const manifestPath = join(modelDirectory, 'model-manifest.json')
...
for (const file of manifest.files) {
  ...
  if (await isVerified(destination, file)) {
    console.info(`[embedding-model] verified ${file.path}`)
    continue
  }
  await mkdir(dirname(destination), { recursive: true, mode: 0o755 })
  const temporary = `${destination}.part`
  await rm(temporary, { force: true })
  console.info(`[embedding-model] downloading ${file.path}`)
...
console.info(`[embedding-model] ready ${manifest.modelId}@${manifest.revision}`)
