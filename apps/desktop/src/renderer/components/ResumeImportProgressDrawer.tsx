...
export function ResumeImportProgressDrawer({ progress, onClose, onOpenCandidates, onOpenFailures }: ResumeImportProgressDrawerProps) {
      ...
      : parsing ? (zh ? '正在本机解析简历' : '端末内で履歴書を解析中')
        : progress.phase === 'partial-failed' ? (zh ? '部分文件未能导入' : '一部のファイルを取り込めませんでした')
          : progress.phase === 'error' ? (zh ? '简历导入未完成' : '履歴書取込を完了できませんでした')
            : (zh ? '人员资料已导入' : '要員情報を取り込みました')
  return <aside aria-label={zh ? '简历导入进度' : '履歴書取込の進捗'} className="resume-import-progress-drawer" role="dialog">
    <header><div><span className="eyebrow">LOCAL RESUME IMPORT</span><h2>{title}</h2><p>{zh ? '导入后可直接找案件或准备介绍；需要补充的内容可在人员资料中修改。' : '取込後は案件探しや紹介の準備ができます。補足は要員情報から編集できます。'}</p></div><button aria-label={zh ? '关闭导入进度' : '取込進捗を閉じる'} disabled={parsing} onClick={onClose} type="button">×</button></header>
    ...
    <ol>{progress.files.map((file) => <li className={`is-${file.status}`} key={file.token}><Icon name={file.status === 'success' ? 'check' : file.status === 'error' ? 'alert' : 'file'} size={16} /><div><strong>{file.name}</strong><small>{file.status === 'queued' ? (zh ? '等待解析' : '解析待ち') : file.status === 'parsing' ? (zh ? '正在解析' : '解析中') : file.status === 'success' ? (zh ? '已导入' : '取込済み') : (file.error ?? (zh ? '解析失败' : '解析に失敗しました'))}</small></div></li>)}</ol>
    {!parsing ? <footer>
      {succeeded.length > 0 ? <button className="is-primary" onClick={onOpenCandidates} type="button">{zh ? '查看人员' : '要員を見る'}<Icon name="chevron-right" size={14} /></button> : null}
