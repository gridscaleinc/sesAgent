  ...
  const renderSchedule = () => {
    ...
    if (selectedInterview?.scheduledAt && !rescheduling) {
      ...
      return <section className="recruiting-schedule-summary">
        ...
        {selectedInterview.meetingUrl ? <div className="recruiting-schedule-summary-detail"><strong>{selectedInterview.meetingMethod === 'google-meet' ? 'Google Meet' : 'Zoom'} {zh ? '会议链接' : '会議リンク'}</strong><span>{selectedInterview.meetingUrl}</span>{selectedInterview.meetingMethod === 'zoom' && openInterviewMeetingInputSchema.safeParse({method:'zoom',url:selectedInterview.meetingUrl}).success ? <button onClick={() => void onOpenZoomMeeting({ url: selectedInterview.meetingUrl! })} type="button">{zh ? '测试打开' : '起動テスト'}</button> : selectedInterview.meetingMethod === 'google-meet' && openInterviewMeetingInputSchema.safeParse({method:'google-meet',url:selectedInterview.meetingUrl}).success && onOpenInterviewMeeting ? <button onClick={() => void onOpenInterviewMeeting({ method: 'google-meet', url: selectedInterview.meetingUrl! })} type="button">{zh ? '测试打开 Google Meet' : 'Google Meetを起動テスト'}</button> : null}</div> : null}
    ...
    return <form className="recruiting-schedule-page" onSubmit={(event) => { event.preventDefault(); void saveSchedule() }}>
      <section><header><h2>{roundLabel}</h2><p>{zh ? '先确认时间、负责人和对应会议方式；保存后进入问题准备。' : '日時、担当者、会議方法を確認してから質問準備へ進みます。'}</p></header><div className="recruiting-form-grid">
        ...
        <label><span>{zh ? '面试官/负责人' : '面談者・担当者'}</span><input onChange={(event) => setSchedule((current) => ({ ...current, interviewer: event.target.value }))} placeholder={zh ? '请输入负责人姓名' : '担当者名を入力'} value={schedule.interviewer} /></label>
        ...
        {schedule.method === 'zoom' ? <label className="is-wide"><span>{zh ? 'Zoom 会议链接' : 'Zoom会議リンク'}</span><div className="recruiting-url-input"><input aria-label={zh ? 'Zoom 会议链接' : 'Zoom会議リンク'} onChange={(event) => setSchedule((current) => ({ ...current, meetingUrl: event.target.value }))} placeholder="https://your-company.zoom.us/j/…" type="text" value={schedule.meetingUrl} />{openInterviewMeetingInputSchema.safeParse({method:'zoom',url:schedule.meetingUrl}).success ? <button onClick={() => void onOpenZoomMeeting({ url: schedule.meetingUrl })} type="button"><Icon name="external-link" size={15} />{zh ? '测试打开' : '起動テスト'}</button> : null}</div><small>{zh ? '仅保存到本地加密数据库，不发送给云端 AI。' : '端末内暗号化DBだけに保存し、Cloud AIへ送信しません。'}</small></label> : null}
        {schedule.method === 'google-meet' ? <label className="is-wide"><span>{zh ? 'Google Meet 会议链接' : 'Google Meetリンク'}</span><div className="recruiting-url-input"><input aria-label={zh ? 'Google Meet 会议链接' : 'Google Meetリンク'} onChange={(event) => setSchedule((current) => ({ ...current, meetingUrl: event.target.value }))} placeholder="https://meet.google.com/…" type="text" value={schedule.meetingUrl} /><button onClick={onOpenIntegrationSettings} type="button"><Icon name="settings" size={15} />{zh ? 'Google Workspace 集成设置' : 'Google Workspace連携設定'}</button></div><small>{zh ? '可先在 Google Workspace 设置中完成日历/Meet 集成，再粘贴或同步会议链接。' : 'Google Workspace設定でカレンダー・Meet連携を完了後、会議リンクを貼り付けまたは同期します。'}</small></label> : null}
      ...
      </div><footer><button onClick={onOpenIntegrationSettings} type="button"><Icon name="settings" size={15} />{zh ? '外部系统集成设置' : '外部システム連携設定'}</button><button className="is-primary" disabled={saving !== null} type="submit">{saving === 'schedule' ? (zh ? '正在保存…' : '保存中…') : (zh ? '保存预约并准备问题' : '予約を保存して質問準備へ')}</button></footer></section>
